...
if _G.Switch_Hub_Series_R then
	...
	function SetDescription(description)
		...
		local abcdef = {Url = "http://localhost:7963/SetDescription?Account="..tostring(game.Players.LocalPlayer.Name), Method = "POST", Body = description}
		request(abcdef)
	end
	function SetAlias(description)
		request = http_request or request or HttpPost
		local abcdef = {Url = "http://localhost:7963/SetAlias?Account="..tostring(game.Players.LocalPlayer.Name), Method = "POST", Body = description}
		request(abcdef)
	end
	function GetAll()
		Old_World = false
		...
		function GetInventory_Name()
			local ReturnText = {}
			for i,v in pairs(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventoryWeapons")) do
				table.insert(ReturnText,v["Name"])
			end
		...
		if game.Players.LocalPlayer.Data.DevilFruit.Value == "" then
			...
			Name_Fruit = result[1].." ("..tostring(GetWeaponMastery("Blox Fruit"))..")"
			...
			for i,v in pairs(game:GetService("ReplicatedStorage").Remotes["CommF_"]:InvokeServer("getInventoryFruits")) do
				if type(v) == "table" then
					if v ~= nil then
						...
						table.insert(ReturnText,result[1])
					end
				end
			end
			if #ReturnText ~= 0 then
		...
		function GetMain()
			...
			table.insert(ReturnText,tostring(game.Players.LocalPlayer.Data.Race.Value)..'('..race_evo..')')
			if Max_Skill then
				local result = {}
				local regex = ("([^%s]+)"):format("-")
				local xp = game.Players.LocalPlayer.Data.DevilFruit.Value
				for each in xp:gmatch(regex) do
					table.insert(result, each)
				end 
				Name_Take = "Full "..result[1]
				table.insert(ReturnText,Name_Take)
			elseif tostring(game.Players.LocalPlayer.Data.DevilFruit.Value) ~= '' then
				if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities") == nil then
					local result = {}
					local regex = ("([^%s]+)"):format("-")
					local xp = game.Players.LocalPlayer.Data.DevilFruit.Value
					...
					Name_Take = "Full "..result[1]
					table.insert(ReturnText,Name_Take)
				elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['TAP'] then
					if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['TAP']['Awakened'] == true then
						Name_Awaken = "5/6"
					elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['C']['Awakened'] == true then
						Name_Awaken = "4/6"
					elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['X']['Awakened'] == true then
					...
					elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['Z']['Awakened'] == true then
						Name_Awaken = "1/6"
					elseif Quest_Dough_Awaken then
						Name_Awaken = "0/6"
					else
						Name_Awaken = 'None.'
					end
					...
					local Name_Text = '['..Name_Awaken..'] '..result[1]
					table.insert(ReturnText,Name_Text)
				elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['F'] then
					if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['C']['Awakened'] == true then
						Name_Awaken = "4/5"
					elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['X']['Awakened'] == true then
						Name_Awaken = "3/5"
					elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['F']['Awakened'] == true then
					...
					local Name_Text = '['..Name_Awaken..'] '..result[1]
					table.insert(ReturnText,Name_Text)
				elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['V'] then
					if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['C']['Awakened'] == true then
						Name_Awaken = "3/4"
					elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities")['X']['Awakened'] == true then
						Name_Awaken = "2/4"
		...
		ReturnText = tostring(GetMain())..' | '..'World '..tostring(Name_World)..' Level: '..
		tostring(game.Players.LocalPlayer.Data.Level.Value).." Beli: "..
		tostring(Abbreviate(game:GetService("Players").LocalPlayer.Data:FindFirstChild("Beli").Value))..
	...
	--[[
	function SheetSend()
		if _G.BestSheet ~= nil then
			--SendGui('✅ Send to Url : '..tostring(_G.BestSheet.Url))
			Old_World = false
			New_World = false
			Three_World = false
			local placeId = game.PlaceId
			...
			function GetAllMelee()
				...
				if GodComplete == true then
					if God_Human_H then
						table.insert(ReturnText,"Godhuman(350)")
					else
						table.insert(ReturnText,"Godhuman")
					end
				end
				if TalComplete == true then
					if Dragon_Talon_H then
						table.insert(ReturnText,"Dragon Talon(350)")
					else
						table.insert(ReturnText,"Dragon Talon")
					end
				end
				if EClawComplete == true then
					if Electric_Claw_H then
						table.insert(ReturnText,"Electric Claw(330)")
					else
						table.insert(ReturnText,"Electric Claw")
					end
				end
				if SharkComplete == true then
					if Sharkman_Karate_H then
						table.insert(ReturnText,"Sharkman Karate(330)")
					else
						table.insert(ReturnText,"Sharkman Karate")
					end
				end
				if DeathComplete == true then
					if Death_Step_H then
						table.insert(ReturnText,"Death Step(400)")
					else
						table.insert(ReturnText,"Death Step")
					end
				end
				if SupComplete == true then
					if Superhuman_H then
						table.insert(ReturnText,"Superhuman(330)")
					else
						table.insert(ReturnText,"Superhuman")
					end
				end
			...
			for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
				...
				if v:IsA('Tool') and tostring(v.ToolTip) == 'Gun' then
					local args = {
						[1] = "StoreItem",
						[2] = v.Name
					}
			...
			function GetSword_Sheet()
				local ReturnText = {}
				local Sword_Get_Mas = {}
				if Cursed_Dual_Katana_Mas then
					table.insert(ReturnText,'Cursed Dual Katana(375)')
					table.insert(Sword_Get_Mas,'Cursed Dual Katana')
				end
				if Hallow_Scryte_Mas then
					table.insert(ReturnText,'Hallow Scythe(350)')
					table.insert(Sword_Get_Mas,'Hallow Scythe')
				end
				if Tushita_Mas then
					table.insert(ReturnText,'Tushita(300)')
					table.insert(Sword_Get_Mas,'Tushita')
				end
				if Yama_Mas then
					table.insert(ReturnText,'Yama(300)')
					table.insert(Sword_Get_Mas,'Yama')
				end
				if Spikey_Trident_Mas then
					table.insert(ReturnText,'Spikey Trident(300)')
					table.insert(Sword_Get_Mas,'Spikey Trident')
				end
				for i,v in pairs(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventory")) do
					if v.Type == 'Sword' and not table.find(Sword_Get_Mas,v.Name) then
						if v.Mastery > 1 then
							local xp = v.Name..'('..v.Mastery..')'
							table.insert(ReturnText,xp)
						 end
					end
				end
				for i,v in pairs(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventory")) do
			...
			function GetGun_Sheet()
				local ReturnText = {}
				local Gun_Get_Mas = {}
				if Soul_Guitar_Mas then
					table.insert(ReturnText,'Soul Guitar(300)')
					table.insert(Gun_Get_Mas,'Soul Guitar')
				end
				if Kabucha_Mas then
					table.insert(ReturnText,'Kabucha(300)')
					table.insert(Gun_Get_Mas,'Kabucha')
				end
				if Acidum_Rifle_Mas then
					table.insert(ReturnText,'Acidum Rifle(300)')
					table.insert(Gun_Get_Mas,'Acidum Rifle')
				end
				for i,v in pairs(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventory")) do
					if v.Type == 'Gun' and table.find(Gun_Get_Mas,v.Name) then
						if v.Mastery > 1 then
							local xp = v.Name..'('..v.Mastery..')'
							table.insert(ReturnText,xp)
						 end
			...
			function GetFruitInU()
				...
				for i,v in pairs(game:GetService("ReplicatedStorage").Remotes["CommF_"]:InvokeServer("getInventoryFruits")) do
					if type(v) == "table" then
						if v ~= nil and v.Price >= 500000 then
							result = {}
							local x = v.Name
							local regex = ("([^%s]+)"):format("-")
							for each in x:gmatch(regex) do
			...
			function GetMain()
				...
				if Max_Skill then
					...
					Name_Take = "Full "..result[1]
					table.insert(ReturnText,Name_Take)
				elseif tostring(game.Players.LocalPlayer.Data.DevilFruit.Value) ~= 'Dough-Dough' and tostring(game.Players.LocalPlayer.Data.DevilFruit.Value) ~= '' then
					if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities") == nil then
						local result = {}
						local regex = ("([^%s]+)"):format("-")
						local xp = game.Players.LocalPlayer.Data.DevilFruit.Value
				...
				if God_Human_Buy then
					table.insert(ReturnText,'Godhuman')
				end
				if Soul_Guitar_H then
					table.insert(ReturnText,'Soul Guitar')
				end
				if Cursed_Dual_Katana_H then
					table.insert(ReturnText,'Cursed Dual Katana')
				end
				if Hallow_Scryte_H then
					table.insert(ReturnText,'Hallow Scythe')
				end
				if Tushita_H then
					table.insert(ReturnText,'Tushita')
				end
				if Yama_H then
					table.insert(ReturnText,'Yama')
				end
				if True_Triple_Katana_H then
					table.insert(ReturnText,'True Triple Katana')
				end
				if Koko_H then
					table.insert(ReturnText,'Koko')
				end
				if #ReturnText ~= 0 then
					return table.concat(ReturnText,", ")
				else
			...
			function GetFruitSheet()
				if Name_Fruit ~= nil then
					if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getAwakenedAbilities") ~= nil then
						local Name_Awaken = ''
						...
						if Name_Awaken == '' then
							...
							local xp = Name_Fruit..' : '..Name_Awaken
							return xp
						end
					end
				end
			...
			Password = '-'
			if not isfile('Switch Hub Id Pass.txt') then
				writefile('Switch Hub Id Pass.txt','Test:Test1')
			end
			if isfile('Switch Hub Id Pass.txt') then
				local input_str = readfile("Switch Hub Id Pass.txt")
			...
			function GetFracta()
				...
				table.insert(ReturnText,Mirror_Fractal_P)
				table.insert(ReturnText,Valkyrie_Helmet_P)
				if #ReturnText ~= 0 then
					return table.concat(ReturnText," | ")
				else
					return ""
			...
			--local url = "https://sheet.best/api/sheets/f31034e4-35f7-4a64-8338-175c77d446ec"
			local data = {
				["Username"] = tostring(game.Players.LocalPlayer.Name),
				["Password"] = Password,
				["World"] = tostring(Name_World),
				...
				['Evo'] = tostring(race_evo),

				['Mirror Fracta | Valkyrie Helmet'] = tostring(GetFracta()),
				['Information'] = tostring(GetMain()), -- Main

				["Fruit"] = tostring(GetFruitSheet()),
				["Rare Fruit"] = tostring(GetFruitInU()),

				["Melee"] = tostring(GetAllMelee()),
				["Sword"] = tostring(GetSword_Sheet()),
				["Gun"] = tostring(GetGun_Sheet()),
				["Wear"] = tostring(GetWear_Sheet())
			}
			...
			request = http_request or request or HttpPost
			local abcdef = {Url = _G.BestSheet['Url'], Body = newdata, Method = "POST", Headers = headers}
			request(abcdef)
			library.Notify({
				Text = ' ✅ Success Api Sheet Best ',
				Time = 1
			})
		end
	end
	...
	spawn(function()
			...
			spawn(function()
				while wait() do
					pcall(function()
						...
						Nexus:NewLine()
						Nexus:CreateLabel("loginfo", "Sheet Api")
						Nexus:CreateTextBox("logurl", _G.BestSheet['Url'], {354, 20})
						Nexus:CreateButton("logsheet", "Sent Log Sheet", {170, 35})
						Nexus:CreateButton("logram", "Sent Log RAM", {170, 35})
						--Nexus:CreateButton("execute", "Execute Capcut", {170, 35})
						--Nexus:CreateButton("shutdown", "Shutdown", {170, 35})
						_G.Bit = true
					end)
				end
			end)
		...
		Nexus:OnButtonClick("logsheet",function()
			_G.BestSheet['Url'] = Nexus:GetText('logurl')
			SheetSend()
		end)
		Nexus:OnButtonClick("logram",function()
			SetDescription(GetAll())
		end)
	end)
	]]
			...
			pcall(function()
				...
				if not List.Quest then -- Melee
					...
					if _G.Melee['Superhuman'] and not Superhuman_H then
						...
						elseif _G.Sword['Cursed Dual Katana'] and Yama_H and Tushita_H and not Cursed_Dual_Katana_H and Three_World then
	...
	while wait() do
		pcall(function()
			...
			if not klmdlkgf and game.Players.LocalPlayer.Data.Level.Value >= 2200 then
				local args = {
					[1] = "BuyHaki",
					[2] = "Geppo"
				}
